...
class Database:
    ...
    def get_model(self, id: Optional[int] = None, name: Optional[str] = None) -> Model:
        if id is not None:
    ...
    def add_market(self, market_name: str):
        ...
        sql = f"ALTER TABLE models ADD COLUMN \"{market_name}\" STRING"
        self.execute(sql, commit=True)

        for model in self.get_models():
            model.add_market(market_name)

    def remove_market(self, market_name):
        if market_name not in self.markets:
            raise ValueError(f"Market \"{market_name}\" does not exist")

        sql = f"ALTER TABLE models DROP COLUMN \"{market_name}\""
        print(sql)
        self.execute(sql, commit=True)

        for model in self.get_models():
            model.remove_market(market_name)
